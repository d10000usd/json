...
def mk_문장분석(text,comtag,serchword):
    ...
    test = '|'.join(serchword)
    print(mdf)
    c=mdf[mdf['text'].str.contains(test)]
    ch = [f{serchword[0]},f{serchword[1]},f{serchword[0]}]
    df.replace(serchword,ch)
    # c 에서 serchword 를 포함한 문장을 yellow로 변경
    #https://fhaktj8-18.tistory.com/entry/%ED%8C%90%EB%8B%A4%EC%8A%A4-pandas-replace-%ED%95%A8%EC%88%98-%EC%B9%98%ED%99%98

    slists = c['text'].to_list()


    test = '|'.join(comtag)
    print(cdf)
    c=cdf[cdf['text'].str.contains(test)]
    clists = c['text'].to_list()


   
    검색어를포함한문장 = "<p>".join(slists)# <br>로 마크다운 줄바꿈
    검색어를포함한문장.replace("이미지","")
    if len(clists) >0:
        회사를포함한문장 = "<p>".join(clists)# <br>로 마크다운 줄바꿈
    else:
        회사를포함한문장 = "관련회사가 없습니다."
